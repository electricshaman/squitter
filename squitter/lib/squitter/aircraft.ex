defmodule Squitter.Aircraft do
  ...
  def start_link(address) do
    GenServer.start_link(
      __MODULE__,
      [address],
      name: {:via, Registry, {Squitter.AircraftRegistry, address}}
    )
  end
  ...
  defp handle_msg(
         %{tc: {:aircraft_id, _}, type_msg: %{aircraft_cat: cat, callsign: callsign}},
         state
       ) do
  ...
  defp handle_msg(
         %{tc: {alt_type, _}, time: time, type_msg: %{flag: flag, alt: alt} = pos},
         state
       )
  ...
  defp handle_msg(%{tc: :air_velocity, type_msg: %GroundSpeed{} = gs}, state) do
    {:ok,
     %{
       state
       | velocity_kt: gs.velocity_kt,
         heading: gs.heading,
         vr: gs.vert_rate,
         vr_src: gs.vert_rate_src
     }}
  end
  ...
  defp handle_msg(%{tc: :air_velocity, type_msg: %AirSpeed{} = as}, state) do
    {:ok,
     %{
       state
       | airspeed_type: as.airspeed_type,
         velocity_kt: as.velocity_kt,
         heading: as.heading,
         vr: as.vert_rate,
         vr_src: as.vert_rate_src
     }}
  end
  ...
  defp handle_msg(
         %{"Lat" => lat, "Long" => lon, "Alt" => alt, "Spd" => spd, "Trak" => trak},
         state
       ) do
    latlon = [lat, lon]
    position = latlon ++ [alt]
  ...
  defp build_view(state) do
    ...
    |> Map.put(:position_history, Enum.reverse(state.position_history))
  ...
  def calculate_position(%{last_even_position: even, last_odd_position: odd} = state)
      when is_nil(even) or is_nil(odd) do
    # We're missing one of the two messages so we can't calculate the position yet
    {:ok, state}
  end

  def calculate_position(
        %{last_even_position: {even_time, even}, last_odd_position: {odd_time, odd}} = state
      ) do
    ...
    with :ok <- check_air_pos_time_delta(even_time, odd_time),
      ...
      {:error, {:air_pos_dist, position, dist}} ->
        Logger.warn(
          "[#{state.address}:#{state.callsign}] Skipping current position #{inspect(position)}: over #{
            @air_pos_dist_delta_max_nm
          } NM from last position of #{inspect(state.latlon)} (#{dist} NM)"
        )
    ...
    case MasterLookup.get(state.address) do
      ...
      {:error, _} ->
        if attempt < @master_attempts do
          ...
          Logger.debug(
            "Scheduling attempt #{next_attempt} to retrieve master data for #{state.address}"
          )
